...
/* к домашнему заданию, п.1 
 оптимизация структуры базы данныз VK*/
...
/* к домашнему заданию, п.2 
добработка структуры БД для возможности использования лайков для медиафайлов, постов и пользователей.
1. для лайков медиафайлов структура БД готова - таблица likes
создадим аналогичные для лайков пользователей и сообщений:
2. для лайков пользователей - таблица likes_users
3. для лайков сообщений - таблица likes_mesages
*/
DROP TABLE IF EXISTS like_users;
CREATE TABLE like_users(
	id SERIAL PRIMARY KEY,
	user_id BIGINT UNSIGNED NOT NULL,
	whom_id BIGINT UNSIGNED NOT NULL, -- кому лайк
...
/*
При таком подходе лайки, относящиеся к разным объектам, будут храниться в разных таблицах.
Возможен подход, при котором все лайки хранились бы в одной таблице likes. В таком случае было бы необходимо лишь 
дополнить иаблицу likes соответствующими полями и связать ограничениями целостности, допускающими NULL. 
Ввиду отсутствия дополнительных вводных, а также из соображений дополнительной практики выбран вариант описанный ваше.

 Помимо этого мои лайки могут поддерживать разные типы для разных объектов) 
